...
S = {
 ...
 # Fade QB/TE in DRAFT CAPITAL but not in ROSTER COUNT: 3 QB and 3 TE, all taken late.
 'Leverage + cover': ['RB','WR','WR','RB','WR','WR','RB','WR','QB','WR','RB','TE','QB','WR','TE','QB','WR','TE'],
 # Same idea, one fewer QB, extra WR.
 'Leverage + 3TE':   ['RB','WR','WR','RB','WR','WR','RB','WR','QB','WR','RB','TE','WR','QB','TE','RB','WR','TE'],
}
NAMES = list(S)

def zero_weeks(roster):
    z=0
    for wk in WEEKS:
        for pos in ('QB','TE'):
            if not any(p['wk'].get(wk) is not None for p in roster if p['pos']==pos): z+=1
    return z
...
def draft(order, sigma):
    ...
    return [score(r) for r in rosters], [zero_weeks(r) for r in rosters]

def experiment(sigma, trials, label):
    tot = {n: [] for n in NAMES}; ZERO={}
    wins = {n: 0 for n in NAMES}
    for t in range(trials):
        order = [NAMES[(i + t) % len(NAMES)] for i in range(TEAMS)]
        res, zs = draft(order, sigma)
        for slot, nm in enumerate(order):
            tot[nm].append(res[slot]); ZERO.setdefault(nm,[]).append(zs[slot])
    ...
    print("%-20s %11s %9s %8s %14s"%("strategy","season pts","vs field","win %","empty QB/TE wks"))
    ...
    for n in rank:
        ...
        print("%-20s %11.0f %+9.0f %7.1f%% %14.2f"%(
            n, m, m-field, 100.0*wins[n]/trials, statistics.mean(ZERO[n])))
    return rank

experiment(0, 18, "TRUE BEST BALL, weekly optimal lineups -- perfect draft ordering\n"
                  "(every strategy sees the same board; only allocation differs)")
random.seed(7)
experiment(9, 300, "SAME, but drafters misrank players (sigma = 9 ranks of error)\n"
